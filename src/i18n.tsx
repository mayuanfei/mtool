import React, { createContext, useContext, useState, useEffect, useCallback } from 'react';

type Language = 'en' | 'zh';

const translations = {
  en: {
    // Sidebar
    'Tools': 'Tools',
    'JSON Formatter': 'JSON Formatter',
    'Text to QR': 'Text to QR',
    'System': 'System',
    'Settings': 'Settings',
    'User': 'User',
    'User profile and preferences.': 'User profile and preferences.',
    'Coming soon': 'Coming soon',

    // Video Tasks
    'Video Tasks': 'Video Tasks',
    'Open authenticated teaching sites, save learning links, and control playback globally.': 'Open authenticated teaching sites, save learning links, and control playback globally.',
    'Open teaching sites with saved links, global speed, and background playback compatibility.': 'Open teaching sites with saved links, global speed, and background playback compatibility.',
    'Please enter a valid teaching URL.': 'Please enter a valid teaching URL.',
    'Teaching site opened in the built-in browser.': 'Teaching site opened in the built-in browser.',
    'Please name this bookmark.': 'Please name this bookmark.',
    'Bookmark saved.': 'Bookmark saved.',
    'Open a teaching site first.': 'Open a teaching site first.',
    'Focus mini window enabled.': 'Focus mini window enabled.',
    'Full-size player restored.': 'Full-size player restored.',
    'Player window closed.': 'Player window closed.',
    'Player window restored.': 'Player window restored.',
    'Bring Player Back': 'Bring Player Back',
    'Teaching Website': 'Teaching Website',
    'Teaching URL': 'Teaching URL',
    'Opening...': 'Opening...',
    'Open Browser': 'Open Browser',
    'Bookmark Name': 'Bookmark Name',
    'e.g. 2026 Security Awareness Training': 'e.g. 2026 Security Awareness Training',
    'Save Bookmark': 'Save Bookmark',
    'Global Playback Speed': 'Global Playback Speed',
    'Applied to videos in the task browser': 'Applied to videos in the task browser',
    'Maximum 10x': 'Maximum 10x',
    'Global Mute': 'Global Mute',
    'Applied to all videos in the task browser': 'Applied to all videos in the task browser',
    'Muted': 'Muted',
    'Sound On': 'Sound On',
    'Player running': 'Player running',
    'Player closed': 'Player closed',
    'Background Playback Compatibility': 'Background Playback Compatibility',
    'Focus mini window': 'Focus mini window',
    'Keeps a small always-on-top WebView visible and filters common blur/visibility pause handlers. This is more reliable than true OS minimization, which may throttle the webpage.': 'Keeps a small always-on-top WebView visible and filters common blur/visibility pause handlers. This is more reliable than true OS minimization, which may throttle the webpage.',
    'Restore Full Window': 'Restore Full Window',
    'Enable Mini Window': 'Enable Mini Window',
    'Close Player': 'Close Player',
    'Saved Learning Links': 'Saved Learning Links',
    'No teaching links saved yet.': 'No teaching links saved yet.',
    'Rename': 'Rename',
    
    // JsonFormatter
    'Clear': 'Clear',
    'Minify': 'Minify',
    'Format JSON': 'Format JSON',
    'Raw Input': 'Raw Input',
    'Formatted Output': 'Formatted Output',
    'Copy': 'Copy',
    'Copied!': 'Copied!',
    'Paste raw JSON here...': 'Paste raw JSON here...',
    'Formatted output will appear here...': 'Formatted output will appear here...',
    'System Ready': 'System Ready',
    'Lines': 'Lines',
    'Length': 'Length',
    'chars': 'chars',
    'Input exceeds 5MB limit. Please provide a smaller JSON.': 'Input exceeds 5MB limit. Please provide a smaller JSON.',
    
    // TextToQr
    'Raw Payload': 'Raw Payload',
    'Enter URL, text, or JSON payload...': 'Enter URL, text, or JSON payload...',
    'Redundancy Level': 'Redundancy Level',
    'Matrix Resolution': 'Matrix Resolution',
    'Chromatic Injection': 'Chromatic Injection',
    'HEX': 'HEX',
    'Preview': 'Preview',
    'Format: PNG': 'Format: PNG',
    'Dimensions': 'Dimensions',
    'Copy Image': 'Copy Image',
    'Download': 'Download',
    'Failed': 'Failed',
    'Enter payload to generate': 'Enter payload to generate',
    'Failed to generate QR Code': 'Failed to generate QR Code',
    'Payload too long (max 2048 chars)': 'Payload too long (max 2048 chars)',
    'Payload is empty': 'Payload is empty',
    'Recent Records': 'Recent Records',
    'Last 10 items': 'Last 10 items',
    'Click to apply': 'Click to apply',
    'No history records yet': 'No history records yet',
    
    // PasswordGenerator
    'Password Generator': 'Password Generator',
    'STRONG': 'STRONG',
    'GOOD': 'GOOD',
    'FAIR': 'FAIR',
    'WEAK': 'WEAK',
    'Password Length': 'Password Length',
    'Uppercase Letters': 'Uppercase Letters',
    'A-Z': 'A-Z',
    'Lowercase Letters': 'Lowercase Letters',
    'a-z': 'a-z',
    'Numbers': 'Numbers',
    '0-9': '0-9',
    'Symbols': 'Symbols',
    '!@#$%^&*': '!@#$%^&*',
    'Exclude Characters': 'Exclude Characters',
    'e.g. iIl1Oo0': 'e.g. iIl1Oo0',
    'Generate Count': 'Generate Count',
    'TIMESTAMP': 'TIMESTAMP',
    'PREVIEW': 'PREVIEW',
    'STRENGTH': 'STRENGTH',
    'ACTION': 'ACTION',
    'Create secure passwords.': 'Create secure passwords.',
    'Please select at least one character type': 'Please select at least one character type',
    'Regenerate': 'Regenerate',
    'Generation History': 'Generation History',
    'Last 5 items': 'Last 5 items',
    
    // SqlInBuilder
    'SQL IN Builder': 'SQL IN Builder',
    'Quote Style': 'Quote Style',
    'Single Quote': 'Single Quote',
    'Double Quote': 'Double Quote',
    'No Quote': 'No Quote',
    'Input Values': 'Input Values',
    'SQL Output': 'SQL Output',
    'Paste one value per line...': 'Paste one value per line...',
    'items': 'items',
    'duplicates removed': 'duplicates removed',
    'SQL IN clause will appear here...': 'SQL IN clause will appear here...',
    'unique values': 'unique values',
    'SQL Preview': 'SQL Preview',
    'Build SQL IN clause from column values.': 'Build SQL IN clause from column values.',
    
    // Settings
    'Index build already in progress': 'Index build already in progress',
    'Configure MTOOL behaviors and active utilities.': 'Configure MTOOL behaviors and active utilities.',
    'Utility Configuration': 'Utility Configuration',
    'Parse, validate, and beautify raw JSON payloads.': 'Parse, validate, and beautify raw JSON payloads.',
    'Generate scannable QR codes from string inputs.': 'Generate scannable QR codes from string inputs.',
    'General Settings': 'General Settings',
    'Language': 'Language',
    'Select application interface language.': 'Select application interface language.',
    'Select a tool from the sidebar': 'Select a tool from the sidebar',
    'Appearance': 'Appearance',
    'Theme': 'Theme',
    'Choose light or dark color scheme.': 'Choose light or dark color scheme.',
    'Dark': 'Dark',
    'Light': 'Light',

    // Updates
    'Updates': 'Updates',
    'Auto-update': 'Auto-update',
    'Automatically check for updates on startup.': 'Automatically check for updates on startup.',
    'Current version': 'Current version',
    'Check for Updates': 'Check for Updates',
    'Up to date': 'Up to date',
    'available': 'available',
    'Install & Restart': 'Install & Restart',
    'Restart Now': 'Restart Now',
    'Ready to restart': 'Ready to restart',
    'Checking...': 'Checking...',
    'Downloading...': 'Downloading...',
    'Update error': 'Update error',
    "What's new in": "What's new in",
    'Cancel': 'Cancel',
    'Skip this version': 'Skip this version',
    'No changelog provided.': 'No changelog provided.',

    // MarkdownEditor
    'Markdown Editor': 'Markdown Editor',
    'View and edit Markdown files with live preview.': 'View and edit Markdown files with live preview.',
    'Editor': 'Editor',
    'Start writing Markdown...': 'Start writing Markdown...',
    'Preview will appear here...': 'Preview will appear here...',
    'Open': 'Open',
    'Save': 'Save',
    'Save As': 'Save As',
    'Untitled': 'Untitled',
    'Modified': 'Modified',
    'Words': 'Words',
    'Edit Mode': 'Edit Mode',
    'Split View': 'Split View',
    'Preview Mode': 'Preview Mode',
    'Collapse Sidebar': 'Collapse Sidebar',
    'Expand Sidebar': 'Expand Sidebar',
    'Release to open file': 'Release to open file',
    'Render error': 'Render error',
    'You have unsaved changes in Markdown Editor. Are you sure you want to leave?': 'You have unsaved changes in Markdown Editor. Are you sure you want to leave?',
    'Outline': 'Outline',
    'Toggle Outline': 'Toggle Outline',

    // FileSearch
    'File Search': 'File Search',
    'Indexing...': 'Indexing...',
    'files indexed': 'files indexed',
    'Re-index': 'Re-index',
    'Results': 'Results',
    'Searching...': 'Searching...',
    'Search and find files by name, size, or content.': 'Search and find files by name, size, or content.',
    'No index data': 'No index data',
    'Click "Re-index" to scan all system files': 'Click "Re-index" to scan all system files',
    'Building index in background...': 'Building index in background...',
    'indexed so far, searchable when done': 'indexed so far, searchable when done',
    'Enter keyword to search files': 'Enter keyword to search files',
    'No matching files found': 'No matching files found',
    'Content search requires a filename or glob pattern, e.g. *.yml content:xxx': 'Content search requires a filename or glob pattern, e.g. *.yml content:xxx',
    'last built': 'last built',
    'Recent Searches': 'Recent Searches',
    'Remove this record': 'Remove this record',
    'Search Results': 'Search Results',
    'entries': 'entries',
    '(first 200 shown)': '(first 200 shown)',
    'Size': 'Size',
    'Created': 'Created',
    'Date Modified': 'Date Modified',
    'Search file names (supports * and ?)...   e.g. *.yml   report draft   size:>100MB   content:"keyword"': 'Search file names (supports * and ?)...   e.g. *.yml   report draft   size:>100MB   content:"keyword"',
    'glob match': 'glob match',
    'size filter': 'size filter',
    'file content search': 'file content search',
    'Reveal in file manager': 'Reveal in file manager',
    'keyword': 'keyword',
    'Failed to open file': 'Failed to open file',

    // FileDiff
    'File Compare': 'File Compare',
    'Compare two text files and highlight differences.': 'Compare two text files and highlight differences.',
    'Original File': 'Original File',
    'Modified File': 'Modified File',
    'Open File': 'Open File',
    'Paste from Clipboard': 'Paste from Clipboard',
    'Pasted Text': 'Pasted Text',
    'Identical': 'Identical',
    'differences': 'differences',
    'Hide Input': 'Hide Input',
    'Show Input': 'Show Input',
    'Previous Difference': 'Previous Difference',
    'Next Difference': 'Next Difference',
    'Swap Files': 'Swap Files',
    'Reset': 'Reset',
    'Load two files to compare': 'Load two files to compare',
    'File size exceeds 10MB limit': 'File size exceeds 10MB limit',
    'Only text files are supported': 'Only text files are supported',
    'Invalid HEX/BASE64 input': 'Invalid HEX/BASE64 input',
    'Pasted text exceeds 10MB limit': 'Pasted text exceeds 10MB limit',
    'Text exceeds 10MB limit': 'Text exceeds 10MB limit',
    'Files exceed 10000 lines. Diff is approximate.': 'Files exceed 10000 lines. Diff is approximate.',
    'Paste text here, or drop a file': 'Paste text here, or drop a file',
    'Wrap Lines': 'Wrap Lines',
    'Unwrap Lines': 'Unwrap Lines',

    // JarViewer
    'Jar Viewer': 'Jar Viewer',
    'View and decompile Java archive files.': 'View and decompile Java archive files.',
    'Jar Viewer & Decompiler': 'Jar Viewer & Decompiler',
    'Drag and drop a .jar, .class, or any text file here, or click the button below to browse.': 'Drag and drop a .jar, .class, or any text file here, or click the button below to browse.',
    'Archive Contents': 'Archive Contents',
    'File': 'File',
    'Decompiling/Reading...': 'Decompiling/Reading...',
    'Loading...': 'Loading...',
    'Select a file to view its contents': 'Select a file to view its contents',
    'Open Another': 'Open Another',
    'Search archive entries...': 'Search archive entries...',
    'No matching archive entries': 'No matching archive entries',
    '{count} matches': '{count} matches',
    'Find in file': 'Find in file',
    'Find in current file...': 'Find in current file...',
    'No matches': 'No matches',
    'Previous match': 'Previous match',
    'Next match': 'Next match',
    'Close search': 'Close search',
    
    // EncoderDecoder
    'Encoder / Decoder': 'Encoder / Decoder',
    'Convert between various data formats.': 'Convert between various data formats.',
    'Mode': 'Mode',
    'Encode': 'Encode',
    'Decode': 'Decode',
    'Base64': 'Base64',
    'URL': 'URL',
    'Unicode': 'Unicode',
    'HTML Entity': 'HTML Entity',
    'XML Entity': 'XML Entity',
    'Input': 'Input',
    'Output': 'Output',
    'Clear Input': 'Clear Input',
    'Copy Output': 'Copy Output',

    // CryptoTool
    'Crypto Tool': 'Crypto Tool',
    'Encrypt, decrypt, and hash data including SM algorithms.': 'Encrypt, decrypt, and hash data including SM algorithms.',
    'Hash / Digest': 'Hash / Digest',
    'Symmetric': 'Symmetric',
    'Asymmetric': 'Asymmetric',
    'HQ DLL': 'Java Dynamic Library Crypto',
    'Hash algorithms cannot be decrypted.': 'Hash algorithms cannot be decrypted.',
    'Key is required.': 'Key is required.',
    'IV is required for CBC mode.': 'IV is required for CBC mode.',
    'Decryption failed. Invalid Key/IV or data.': 'Decryption failed. Invalid Key/IV or data.',
    'Public Key is required for encryption.': 'Public Key is required for encryption.',
    'RSA Encryption failed.': 'RSA Encryption failed.',
    'Private Key is required for decryption.': 'Private Key is required for decryption.',
    'RSA Decryption failed.': 'RSA Decryption failed.',
    'SM2 Decryption failed.': 'SM2 Decryption failed.',
    'SM2 Encryption failed.': 'SM2 Encryption failed.',
    'Key Size': 'Key Size',
    'padded': 'padded',
    'truncated': 'truncated',
    'Key length mismatch (current: {current}B), it will be {action} to {target}B.': 'Key length mismatch (current: {current}B), it will be {action} to {target}B.',
    'IV length mismatch (current: {current}B), it will be {action} to {target}B.': 'IV length mismatch (current: {current}B), it will be {action} to {target}B.',
    'Invalid key format': 'Invalid key format',
    'Invalid IV format': 'Invalid IV format',
    'Generating...': 'Generating...',
    'Keys Configuration': 'Keys Configuration',
    'Auto Generate Key Pair': 'Auto Generate Key Pair',
    'Public Key': 'Public Key',
    'Private Key': 'Private Key',
    'This feature requires the HQ DLL installed in C:\\Windows\\System32. Only available on Windows environments.': 'This feature requires the HQ DLL installed in C:\\Windows\\System32. Only available on Windows environments.',
    'Please select the HQ Jar file.': 'Please select the HQ Jar file.',
    'Jar Path': 'Jar Path',
    'Select the ums-sm4.jar file (Natives directory must be present)': 'Select ums-sm4.jar (Ensure native library is in java.library.path)',
    'Browse': 'Browse',
    'JDK Path': 'JDK Path',
    'Optional: select JDK/JRE directory (default: system PATH)': 'Optional: select JDK/JRE directory (default: system PATH)',
    'Custom Parameter': 'Custom Parameter',
    'Batch decrypt: one ciphertext per line, up to 1000 lines; up to 5 Java processes run concurrently.': 'Batch decrypt: one ciphertext per line, up to 1000 lines; up to 5 Java processes run concurrently.',
    'HQ DLL batch supports at most {max} lines.': 'HQ DLL batch supports at most {max} lines.',
    'HQ DLL batch input line {line} is empty.': 'HQ DLL batch input line {line} is empty.',
    '[Line {line} failed] {error}': '[Line {line} failed] {error}',
    'Batch decryption finished with {count} failed line(s).': 'Batch decryption finished with {count} failed line(s).',
    'Address (E001)': 'Address (E001)',
    'Track / Bin (F001)': 'Track / Bin (F001)',
    'Hash': 'Hash',
    'Encrypt': 'Encrypt',
    'Decrypt': 'Decrypt',
    'Key': 'Key',
    'IV': 'IV',
    'Not needed for ECB': 'Not needed for ECB',
    'Public Key Encrypt': 'Public Key Encrypt',
    'Private Key Decrypt': 'Private Key Decrypt',
    'Padding': 'Padding',
    'Format': 'Format',
    'UTF8': 'UTF-8',
    'BASE64': 'Base64',
    'Data length must be a multiple of 16 bytes for NoPadding.': 'Data length must be a multiple of 16 bytes for NoPadding.',
    'RSA plaintext is too long ({len} bytes). For {size}-bit key, the maximum is {max} bytes.': 'RSA plaintext is too long ({len} bytes). For {size}-bit key, the maximum is {max} bytes.',
    'Cipher Mode': 'Cipher Mode',
    'Decryption requires binary ciphertext. Please select HEX or BASE64 as the input format.': 'Decryption requires binary ciphertext. Please select HEX or BASE64 as the input format.',
    
    // ErrorBoundary
    'Failed to load components, please restart the app or refresh': 'Failed to load components, please restart the app or refresh',
    'Unknown error occurred': 'Unknown error occurred',
    'Reload': 'Reload',

    // File Transfer
    'File Transfer': 'File Transfer',
    'Lan File Transfer': 'LAN File Transfer',
    'Transfer files securely and fast within your local network.': 'Transfer files securely and fast within your local network.',
    'My Device': 'My Device',
    'Local IPs': 'Local IPs',
    'Port': 'Port',
    'Save Path': 'Save Path',
    'Default (Downloads)': 'Default (Downloads)',
    'Change Path': 'Change Path',
    'Reset to Default': 'Reset to Default',
    'Add Peer': 'Add Peer',
    'Peer IP': 'Peer IP',
    'Alias (Optional)': 'Alias (Optional)',
    'Friend Request Sent': 'Friend Request Sent',
    'Friend Request Accepted': 'Friend Request Accepted',
    'Friend Request Rejected': 'Friend Request Rejected',
    'Failed to send request': 'Failed to send request',
    'Trusted Peers': 'Trusted Peers',
    'No trusted peers yet. Add a peer by IP above!': 'No trusted peers yet. Add a peer by IP above!',
    'Send File': 'Send File',
    'Select File': 'Select File',
    'Drag and drop a file here, or click to browse.': 'Drag and drop a file here, or click to browse.',
    'Selected File': 'Selected File',
    'Target Peer': 'Target Peer',
    'Choose a peer...': 'Choose a peer...',
    'Start Transfer': 'Start Transfer',
    'Transmissions': 'Transmissions',
    'History': 'History',
    'No active transfers.': 'No active transfers.',
    'No history records.': 'No history records.',
    'Incoming Friend Request': 'Incoming Friend Request',
    'wants to add you as a peer. Agree?': 'wants to add you as a peer. Agree?',
    'Accept': 'Accept',
    'Reject': 'Reject',
    'Show in Finder': 'Show in Finder',
    'Delete': 'Delete',
    'Delete Friend': 'Delete Friend',
    'Edit Alias': 'Edit Alias',
    'Delete History': 'Delete History',
    'Clear History': 'Clear History',
    'Are you sure you want to remove this peer?': 'Are you sure you want to remove this peer?',
    'Are you sure you want to delete this history record?': 'Are you sure you want to delete this history record?',
    'Are you sure you want to clear all history records?': 'Are you sure you want to clear all history records?',
    'Confirm': 'Confirm',
    'Receive failed': 'Receive failed',
    
    // Pandoc Document Converter
    'Doc Converter': 'Doc Converter',
    'Convert document formats using Pandoc.': 'Convert document formats using Pandoc.',
    'Pandoc Status': 'Pandoc Status',
    'Detecting...': 'Detecting...',
    'Detected': 'Detected',
    'Not Installed': 'Not Installed',
    'Source File': 'Source File',
    'Target File': 'Target File',
    'From Format': 'From Format',
    'To Format': 'To Format',
    'Convert': 'Convert',
    'Converting...': 'Converting...',
    'Conversion Successful!': 'Conversion Successful!',
    'Conversion Failed': 'Conversion Failed',
    'Show in Folder': 'Show in Folder',
    'Custom Arguments': 'Custom Arguments',
    'e.g., --pdf-engine=xelatex': 'e.g., --pdf-engine=xelatex',
    'Pandoc is not installed on your system. Please follow the instructions below to install it:': 'Pandoc is not installed on your system. Please follow the instructions below to install it:',
    'macOS Installation': 'macOS: Run \'brew install pandoc\' in your terminal.',
    'Windows Installation': 'Windows: Run \'winget install jgm.pandoc\' in Command Prompt.',
    'Linux Installation': 'Linux: Use your package manager, e.g., \'sudo apt install pandoc\'.',
    'Auto Detect': 'Auto Detect',
    'Initializing download...': 'Initializing download...',
    'Extracting binary...': 'Extracting binary...',
    'Pandoc installed successfully!': 'Pandoc installed successfully!',
    'Install Pandoc': 'Install Pandoc',
    'Downloading necessary plugins...': 'Downloading necessary plugins...',
    'Necessary plugin (Pandoc) is missing. Click \'Install\' to automatically download and configure it. You can also install it manually on your system.': 'Necessary plugin (Pandoc) is missing. Click \'Install\' to automatically download and configure it. You can also install it manually on your system.',
    'Install Plugin': 'Install Plugin',
    'Time': 'Time',
    'Input File': 'Input File',
    'Output File': 'Output File',
    'Status': 'Status',
    'Actions': 'Actions',
    'Success': 'Success',
    'No conversion history yet.': 'No conversion history yet.',
    'Recent History': 'Recent History',
    'Please select source file': 'Please select source file',
    'Please select target path': 'Please select target path',
    'Converting to PDF requires a PDF engine on your system (defaults to pdflatex). If you encounter a pdflatex missing error, you can specify an alternative engine in "Custom Arguments" below (e.g. --pdf-engine=wkhtmltopdf or --pdf-engine=typst). Note: For Typst 0.14+, you may also need to append a main font argument, e.g., -V mainfont="Microsoft YaHei" or -V mainfont="Arial".': 'Converting to PDF requires a PDF engine on your system (defaults to pdflatex). If you encounter a pdflatex missing error, you can specify an alternative engine in "Custom Arguments" below (e.g. --pdf-engine=wkhtmltopdf or --pdf-engine=typst). Note: For Typst 0.14+, you may also need to append a main font argument, e.g., -V mainfont="Microsoft YaHei" or -V mainfont="Arial".',
    'Plugin Installation Failed': 'Plugin Installation Failed',
    'Specify command line arguments for Pandoc. Use spaces to split multiple flags.': 'Specify command line arguments for Pandoc. Use spaces to split multiple flags.',
  },
  zh: {
    // Sidebar
    'Tools': '工具',
    'JSON Formatter': 'JSON 格式化',
    'Text to QR': '文本转二维码',
    'System': '系统',
    'Settings': '设置',
    'User': '用户',
    'User profile and preferences.': '用户资料与偏好设置。',
    'Coming soon': '即将推出',

    // Video Tasks
    'Video Tasks': '视频任务',
    'Open authenticated teaching sites, save learning links, and control playback globally.': '打开需要登录的教学网站，收藏学习链接并统一控制播放倍速。',
    'Open teaching sites with saved links, global speed, and background playback compatibility.': '通过收藏链接、全局倍速和后台播放兼容模式打开教学网站。',
    'Please enter a valid teaching URL.': '请输入有效的教学网址。',
    'Teaching site opened in the built-in browser.': '已在内置浏览器中打开教学网站。',
    'Please name this bookmark.': '请为这个收藏起一个名称。',
    'Bookmark saved.': '网址收藏已保存。',
    'Open a teaching site first.': '请先打开一个教学网站。',
    'Focus mini window enabled.': '已启用专注小窗。',
    'Full-size player restored.': '已恢复完整播放窗口。',
    'Player window closed.': '播放窗口已关闭。',
    'Player window restored.': '播放窗口已唤回。',
    'Bring Player Back': '唤回播放窗口',
    'Teaching Website': '教学网站',
    'Teaching URL': '教学网址',
    'Opening...': '正在打开...',
    'Open Browser': '打开浏览器',
    'Bookmark Name': '收藏名称',
    'e.g. 2026 Security Awareness Training': '例如：2026年安全意识培训',
    'Save Bookmark': '保存收藏',
    'Global Playback Speed': '全局播放倍速',
    'Applied to videos in the task browser': '自动应用到任务浏览器中的视频',
    'Maximum 10x': '最高 10 倍速',
    'Global Mute': '全局静音',
    'Applied to all videos in the task browser': '自动应用到任务浏览器中的所有视频',
    'Muted': '已静音',
    'Sound On': '声音开启',
    'Player running': '播放窗口运行中',
    'Player closed': '播放窗口未打开',
    'Background Playback Compatibility': '后台播放兼容',
    'Focus mini window': '专注小窗',
    'Keeps a small always-on-top WebView visible and filters common blur/visibility pause handlers. This is more reliable than true OS minimization, which may throttle the webpage.': '保持一个很小的置顶网页视图，并过滤常见的失焦和页面不可见暂停逻辑。它比真正最小化更可靠，因为系统可能会限制已最小化网页的运行。',
    'Restore Full Window': '恢复完整窗口',
    'Enable Mini Window': '启用专注小窗',
    'Close Player': '关闭播放窗口',
    'Saved Learning Links': '已收藏的学习网址',
    'No teaching links saved yet.': '还没有收藏教学网址。',
    'Rename': '重命名',
    
    // JsonFormatter
    'Clear': '清空',
    'Minify': '压缩',
    'Format JSON': '格式化 JSON',
    'Raw Input': '原始输入',
    'Formatted Output': '格式化输出',
    'Copy': '复制',
    'Copied!': '已复制!',
    'Paste raw JSON here...': '在此粘贴原始 JSON...',
    'Formatted output will appear here...': '格式化后的输出将显示在此处...',
    'System Ready': '系统就绪',
    'Lines': '行数',
    'Length': '长度',
    'chars': '字符',
    'Input exceeds 5MB limit. Please provide a smaller JSON.': '输入超过 5MB 限制。请提供较小的 JSON。',
    
    // TextToQr
    'Raw Payload': '原始文本',
    'Enter URL, text, or JSON payload...': '输入网址、文本或 JSON...',
    'Redundancy Level': '容错级别',
    'Matrix Resolution': '图像分辨率',
    'Chromatic Injection': '颜色配置',
    'HEX': 'HEX',
    'Preview': '预览',
    'Format: PNG': '格式: PNG',
    'Dimensions': '尺寸',
    'Copy Image': '复制图片',
    'Download': '下载',
    'Failed': '操作失败',
    'Enter payload to generate': '输入文本以生成',
    'Failed to generate QR Code': '生成二维码失败',
    'Payload too long (max 2048 chars)': '文本过长（最多支持 2048 个字符）',
    'Payload is empty': '输入文本不能为空',
    'Recent Records': '最近记录',
    'Last 10 items': '最近 10 条记录',
    'Click to apply': '点击应用',
    'No history records yet': '暂无历史记录',
    
    // PasswordGenerator
    'Password Generator': '密码生成器',
    'STRONG': '强',
    'GOOD': '中等',
    'FAIR': '较弱',
    'WEAK': '弱',
    'Password Length': '密码长度',
    'Uppercase Letters': '大写字母',
    'A-Z': 'A-Z',
    'Lowercase Letters': '小写字母',
    'a-z': 'a-z',
    'Numbers': '数字',
    '0-9': '0-9',
    'Symbols': '特殊符号',
    '!@#$%^&*': '!@#$%^&*',
    'Exclude Characters': '排除字符',
    'e.g. iIl1Oo0': '例如：iIl1Oo0',
    'Generate Count': '生成数量',
    'TIMESTAMP': '时间',
    'PREVIEW': '预览',
    'STRENGTH': '强度',
    'ACTION': '操作',
    'Create secure passwords.': '创建安全的密码。',
    'Please select at least one character type': '请至少选择一种字符类型',
    'Regenerate': '重新生成',
    'Generation History': '生成历史',
    'Last 5 items': '最近 5 条记录',
    
    // SqlInBuilder
    'SQL IN Builder': 'SQL IN 构建器',
    'Quote Style': '引号风格',
    'Single Quote': '单引号',
    'Double Quote': '双引号',
    'No Quote': '无引号',
    'Input Values': '输入值',
    'SQL Output': 'SQL 输出',
    'Paste one value per line...': '每行粘贴一个值...',
    'items': '项',
    'duplicates removed': '个重复项已去除',
    'SQL IN clause will appear here...': 'SQL IN 子句将显示在此处...',
    'unique values': '个唯一值',
    'SQL Preview': 'SQL 预览',
    'Build SQL IN clause from column values.': '将一列值组装为 SQL IN 子句。',
    
    // Settings
    'Index build already in progress': '索引重建正在进行中',
    'Configure MTOOL behaviors and active utilities.': '配置 MTOOL 的行为和启用的工具。',
    'Utility Configuration': '工具配置',
    'Parse, validate, and beautify raw JSON payloads.': '解析、验证并美化原始 JSON 文本。',
    'Generate scannable QR codes from string inputs.': '将文本转换为可扫描的二维码图片。',
    'General Settings': '常规设置',
    'Language': '语言',
    'Select application interface language.': '选择应用程序界面语言。',
    'Select a tool from the sidebar': '请从左侧边栏选择一个工具',
    'Appearance': '外观',
    'Theme': '主题',
    'Choose light or dark color scheme.': '选择明亮或暗黑色彩方案。',
    'Dark': '暗黑',
    'Light': '明亮',

    // Updates
    'Updates': '更新',
    'Auto-update': '自动更新',
    'Automatically check for updates on startup.': '启动时自动检查更新。',
    'Current version': '当前版本',
    'Check for Updates': '检查更新',
    'Up to date': '已是最新版本',
    'available': '可更新',
    'Install & Restart': '安装并重启',
    'Restart Now': '立即重启',
    'Ready to restart': '准备重启',
    'Checking...': '检查中...',
    'Downloading...': '下载中...',
    'Update error': '更新出错',
    "What's new in": '新版本内容',
    'Cancel': '取消',
    'Skip this version': '跳过此版本',
    'No changelog provided.': '暂无更新日志。',

    // MarkdownEditor
    'Markdown Editor': 'Markdown 编辑器',
    'View and edit Markdown files with live preview.': '查看和编辑 Markdown 文件，支持实时预览。',
    'Editor': '编辑器',
    'Start writing Markdown...': '开始编写 Markdown...',
    'Preview will appear here...': '预览将显示在此处...',
    'Open': '打开',
    'Save': '保存',
    'Save As': '另存为',
    'Untitled': '未命名',
    'Modified': '已修改',
    'Words': '单词',
    'Edit Mode': '编辑模式',
    'Split View': '分栏视图',
    'Preview Mode': '预览模式',
    'Collapse Sidebar': '收起侧边栏',
    'Expand Sidebar': '展开侧边栏',
    'Release to open file': '释放以打开文件',
    'Render error': '渲染出错',
    'You have unsaved changes in Markdown Editor. Are you sure you want to leave?': 'Markdown 编辑器中有未保存的修改，确定要离开吗？',
    'Outline': '大纲',
    'Toggle Outline': '切换大纲',

    // FileSearch
    'File Search': '文件搜索',
    'Indexing...': '正在建立索引...',
    'files indexed': '个文件已索引',
    'Re-index': '重新索引',
    'Results': '搜索结果',
    'Searching...': '搜索中...',
    'Search and find files by name, size, or content.': '按文件名、大小或内容搜索文件。',
    'No index data': '暂无索引数据',
    'Click "Re-index" to scan all system files': '点击「重建索引」开始扫描全系统文件',
    'Building index in background...': '正在后台建立索引...',
    'indexed so far, searchable when done': '条，完成后可搜索',
    'Enter keyword to search files': '输入关键词搜索文件',
    'No matching files found': '未找到匹配的文件',
    'Content search requires a filename or glob pattern, e.g. *.yml content:xxx': 'content 搜索需要同时指定文件名或扩展名，例如：*.yml content:xxx',
    'last built': '上次建立',
    'Recent Searches': '最近搜索',
    'Remove this record': '删除此记录',
    'Search Results': '搜索结果',
    'entries': '条',
    '(first 200 shown)': '（仅显示前 200 条）',
    'Size': '大小',
    'Created': '创建',
    'Date Modified': '修改',
    'Search file names (supports * and ?)...   e.g. *.yml   report draft   size:>100MB   content:"keyword"': '搜索文件名(支持*和?)...   例: *.yml   report draft   size:>100MB   content:"关键词"',
    'glob match': 'glob 匹配',
    'size filter': '大小过滤',
    'file content search': '文件内容搜索',
    'Reveal in file manager': '在文件管理器中定位',
    'keyword': '关键词',
    'Failed to open file': '打开文件失败',

    // FileDiff
    'File Compare': '文件对比',
    'Compare two text files and highlight differences.': '对比两个文本文件并高亮显示差异。',
    'Original File': '原始文件',
    'Modified File': '修改后文件',
    'Open File': '打开文件',
    'Paste from Clipboard': '从剪贴板粘贴',
    'Pasted Text': '粘贴的文本',
    'Identical': '完全一致',
    'differences': '处差异',
    'Hide Input': '隐藏输入',
    'Show Input': '显示输入',
    'Previous Difference': '上一个差异',
    'Next Difference': '下一个差异',
    'Swap Files': '交换文件',
    'Reset': '重置',
    'Load two files to compare': '加载两个文件进行对比',
    'File size exceeds 10MB limit': '文件超过 10MB 限制',
    'Only text files are supported': '仅支持文本文件',
    'Invalid HEX/BASE64 input': 'HEX/BASE64 输入格式不合法',
    'Pasted text exceeds 10MB limit': '粘贴的文本超过 10MB 限制',
    'Text exceeds 10MB limit': '文本超过 10MB 限制',
    'Files exceed 10000 lines. Diff is approximate.': '文件超过 10000 行，仅显示粗略差异结果',
    'Paste text here, or drop a file': '在此粘贴文本，或拖拽文件到此处',
    'Wrap Lines': '折行显示',
    'Unwrap Lines': '单行显示',

    // JarViewer
    'Jar Viewer': 'Jar 浏览器',
    'View and decompile Java archive files.': '浏览和反编译 Java 归档文件。',
    'Jar Viewer & Decompiler': 'Jar 浏览器与反编译',
    'Drag and drop a .jar, .class, or any text file here, or click the button below to browse.': '拖拽 .jar、.class 或任意文本文件到此处，或点击下方按钮选择文件。',
    'Archive Contents': '归档内容',
    'File': '文件',
    'Decompiling/Reading...': '反编译 / 读取中...',
    'Loading...': '加载中...',
    'Select a file to view its contents': '选择一个文件以查看其内容',
    'Open Another': '打开其他文件',
    'Search archive entries...': '搜索归档条目...',
    'No matching archive entries': '未找到匹配的归档条目',
    '{count} matches': '{count} 个匹配',
    'Find in file': '在文件中查找',
    'Find in current file...': '在当前文件中查找...',
    'No matches': '无匹配',
    'Previous match': '上一个匹配',
    'Next match': '下一个匹配',
    'Close search': '关闭查找',
    
    // EncoderDecoder
    'Encoder / Decoder': '编码 / 解码',
    'Convert between various data formats.': '在不同数据格式之间进行转换。',
    'Mode': '模式',
    'Encode': '编码',
    'Decode': '解码',
    'Base64': 'Base64',
    'URL': 'URL',
    'Unicode': 'Unicode',
    'HTML Entity': 'HTML 实体',
    'XML Entity': 'XML 实体',
    'Input': '输入',
    'Output': '输出',
    'Clear Input': '清空输入',
    'Copy Output': '复制输出',

    // CryptoTool
    'Crypto Tool': '加解密工具',
    'Encrypt, decrypt, and hash data including SM algorithms.': '提供包含国密算法在内的数据加密、解密与哈希功能。',
    'Hash / Digest': '哈希 / 摘要',
    'Symmetric': '对称加密',
    'Asymmetric': '非对称加密',
    'HQ DLL': 'Java 动态链接库加密',
    'Hash algorithms cannot be decrypted.': '哈希算法是单向的，无法解密。',
    'Key is required.': '密钥不能为空。',
    'IV is required for CBC mode.': 'CBC 模式下偏移量(IV)不能为空。',
    'Decryption failed. Invalid Key/IV or data.': '解密失败：请检查密钥、IV 或密文是否正确。',
    'Public Key is required for encryption.': '加密操作需要公钥。',
    'RSA Encryption failed.': 'RSA 加密失败。',
    'Private Key is required for decryption.': '解密操作需要私钥。',
    'RSA Decryption failed.': 'RSA 解密失败。',
    'SM2 Decryption failed.': 'SM2 解密失败。',
    'SM2 Encryption failed.': 'SM2 加密失败。',
    'Key Size': '密钥长度',
    'padded': '补零',
    'truncated': '截断',
    'Key length mismatch (current: {current}B), it will be {action} to {target}B.': '密钥长度不符（当前为 {current} 字节），将被{action}至 {target} 字节。',
    'IV length mismatch (current: {current}B), it will be {action} to {target}B.': '偏移量(IV)长度不符（当前为 {current} 字节），将被{action}至 {target} 字节。',
    'Invalid key format': '密钥格式无效',
    'Invalid IV format': '偏移量(IV)格式无效',
    'Generating...': '生成中...',
    'Keys Configuration': '密钥配置',
    'Auto Generate Key Pair': '一键生成密钥对',
    'Public Key': '公钥',
    'Private Key': '私钥',
    'This feature requires the HQ DLL installed in C:\\Windows\\System32. Only available on Windows environments.': '该功能需要总公司的专用 DLL 文件放置在 C:\\Windows\\System32 下。仅支持在 Windows 环境中使用。',
    'Please select the HQ Jar file.': '请选择总公司 Jar 包。',
    'Jar Path': 'Jar 文件路径',
    'Select the ums-sm4.jar file (Natives directory must be present)': '选择 ums-sm4.jar（须确保动态库在 java.library.path 中）',
    'Browse': '浏览',
    'JDK Path': 'JDK 目录',
    'Optional: select JDK/JRE directory (default: system PATH)': '可选：指定 JDK/JRE 目录（默认使用系统 PATH）',
    'Custom Parameter': '自定义参数',
    'Batch decrypt: one ciphertext per line, up to 1000 lines; up to 5 Java processes run concurrently.': '批量解密时每行输入一条密文，最多 1000 行；同时最多运行 5 个 Java 进程。',
    'HQ DLL batch supports at most {max} lines.': 'Java 动态链接库批量解密最多支持 {max} 行。',
    'HQ DLL batch input line {line} is empty.': 'Java 动态链接库批量解密的第 {line} 行为空。',
    '[Line {line} failed] {error}': '[第 {line} 行失败] {error}',
    'Batch decryption finished with {count} failed line(s).': '批量解密完成，其中 {count} 行失败。',
    'Address (E001)': '地址 (E001)',
    'Track / Bin (F001)': '磁道信息 (F001)',
    'Hash': '计算',
    'Encrypt': '加密',
    'Decrypt': '解密',
    'Key': '密钥',
    'IV': '偏移量(IV)',
    'Not needed for ECB': 'ECB模式无需IV',
    'Public Key Encrypt': '公钥加密',
    'Private Key Decrypt': '私钥解密',
    'Padding': '填充 (Padding)',
    'Format': '编码',
    'UTF8': '文本 (UTF-8)',
    'BASE64': 'Base64',
    'Data length must be a multiple of 16 bytes for NoPadding.': 'NoPadding 模式下，数据长度必须为 16 字节的倍数。',
    'RSA plaintext is too long ({len} bytes). For {size}-bit key, the maximum is {max} bytes.': 'RSA 明文过长（当前为 {len} 字节）。对于 {size} 位密钥，最大长度为 {max} 字节。',
    'Cipher Mode': '密文排列方式',
    'Decryption requires binary ciphertext. Please select HEX or BASE64 as the input format.': '解密操作需要二进制密文，请选择 HEX 或 BASE64 作为输入编码。',
    
    // ErrorBoundary
    'Failed to load components, please restart the app or refresh': '加载失败，请重启应用或刷新',
    'Unknown error occurred': '发生未知错误',
    'Reload': '重新加载',

    // File Transfer
    'File Transfer': '局域网传输',
    'Lan File Transfer': '局域网文件传输',
    'Transfer files securely and fast within your local network.': '在局域网内安全、快速地传输文件。',
    'My Device': '我的设备',
    'Local IPs': '本机 IP',
    'Port': '监听端口',
    'Save Path': '保存路径',
    'Default (Downloads)': '系统默认下载目录',
    'Change Path': '更改路径',
    'Reset to Default': '恢复默认',
    'Add Peer': '添加对象',
    'Peer IP': '对方 IP',
    'Alias (Optional)': '别名（可选）',
    'Friend Request Sent': '配对请求已发送，等待对方确认...',
    'Friend Request Accepted': '配对请求已被接受！',
    'Friend Request Rejected': '配对请求已被拒绝！',
    'Failed to send request': '发送配对请求失败，请检查 IP 和端口是否正确。',
    'Trusted Peers': '已配对的好友',
    'No trusted peers yet. Add a peer by IP above!': '暂无配对好友。在上方输入 IP 进行添加吧！',
    'Send File': '发送文件',
    'Select File': '选择文件',
    'Drag and drop a file here, or click to browse.': '拖放文件到此处，或点击选择文件。',
    'Selected File': '已选文件',
    'Target Peer': '接收对象',
    'Choose a peer...': '请选择一个好友...',
    'Start Transfer': '开始发送',
    'Transmissions': '当前传输',
    'History': '历史记录',
    'No active transfers.': '暂无进行中的传输。',
    'No history records.': '暂无传输记录。',
    'Incoming Friend Request': '收到配对申请',
    'wants to add you as a peer. Agree?': '申请与您配对为传输好友。是否同意？',
    'Accept': '同意',
    'Reject': '拒绝',
    'Show in Finder': '定位文件',
    'Delete': '删除',
    'Delete Friend': '删除好友',
    'Edit Alias': '修改别名',
    'Delete History': '删除历史记录',
    'Clear History': '清空历史记录',
    'Are you sure you want to remove this peer?': '确定要删除该配对好友吗？',
    'Are you sure you want to delete this history record?': '确定要删除此条传输记录吗？',
    'Are you sure you want to clear all history records?': '确定要清空所有传输历史记录吗？',
    'Confirm': '确认',
    'Receive failed': '接收失败',

    // Pandoc Document Converter
    'Doc Converter': '文档转换器',
    'Convert document formats using Pandoc.': '使用 Pandoc 在多种文档格式之间进行互转。',
    'Pandoc Status': 'Pandoc 状态',
    'Detecting...': '正在检测...',
    'Detected': '已检测到',
    'Not Installed': '未安装',
    'Source File': '源文件',
    'Target File': '目标路径',
    'From Format': '源格式',
    'To Format': '目标格式',
    'Convert': '开始转换',
    'Converting...': '转换中...',
    'Conversion Successful!': '转换成功！',
    'Conversion Failed': '转换失败',
    'Show in Folder': '在文件夹中显示',
    'Custom Arguments': '自定义参数',
    'e.g., --pdf-engine=xelatex': '例如：--pdf-engine=xelatex',
    'Pandoc is not installed on your system. Please follow the instructions below to install it:': '您的系统上未安装 Pandoc。请按照以下说明安装它：',
    'macOS Installation': 'macOS：在终端运行 \'brew install pandoc\'。',
    'Windows Installation': 'Windows：在命令行运行 \'winget install jgm.pandoc\'。',
    'Linux Installation': 'Linux：使用包管理器，例如 \'sudo apt install pandoc\'。',
    'Auto Detect': '自动推断',
    'Initializing download...': '正在初始化下载...',
    'Extracting binary...': '正在解压缩二进制文件...',
    'Pandoc installed successfully!': 'Pandoc 插件安装成功！',
    'Install Pandoc': '安装 Pandoc 插件',
    'Downloading necessary plugins...': '正在下载必要的插件...',
    'Necessary plugin (Pandoc) is missing. Click \'Install\' to automatically download and configure it. You can also install it manually on your system.': '系统未检测到 Pandoc 插件。点击“安装插件”将自动下载并为您配置，您也可以根据提示在系统上手动安装它。',
    'Install Plugin': '安装插件',
    'Time': '时间',
    'Input File': '输入文件',
    'Output File': '输出文件',
    'Status': '状态',
    'Actions': '操作',
    'Success': '成功',
    'No conversion history yet.': '暂无转换历史记录。',
    'Recent History': '最近转换记录',
    'Please select source file': '请选择要转换的源文件',
    'Please select target path': '请选择目标保存路径',
    'Plugin Installation Failed': '插件安装失败',
    'Specify command line arguments for Pandoc. Use spaces to split multiple flags.': '指定 Pandoc 命令行参数，使用空格分隔多个参数。',
    'Converting to PDF requires a PDF engine on your system (defaults to pdflatex). If you encounter a pdflatex missing error, you can specify an alternative engine in "Custom Arguments" below (e.g. --pdf-engine=wkhtmltopdf or --pdf-engine=typst). Note: For Typst 0.14+, you may also need to append a main font argument, e.g., -V mainfont="Microsoft YaHei" or -V mainfont="Arial".': '转换为 PDF 格式需要系统安装有 PDF 引擎（默认为 pdflatex）。若遇到 pdflatex 缺失错误，可在下方【自定义参数】中指定其它引擎（如 --pdf-engine=wkhtmltopdf 或 --pdf-engine=typst）。注意：使用 Typst 转换时，可能还需要追加主字体参数（例如：-V mainfont="Microsoft YaHei" 或 -V mainfont="Arial"）以避免 font fallback 报错。',
  }
};

export type TranslationKey = keyof typeof translations.en;

type I18nContextType = {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: TranslationKey, params?: Record<string, string | number>) => string;
};

const I18nContext = createContext<I18nContextType | undefined>(undefined);

export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [language, setLanguage] = useState<Language>(() => {
    const saved = localStorage.getItem('mtool_language');
    return (saved === 'zh' || saved === 'en') ? saved : 'en';
  });

  useEffect(() => {
    localStorage.setItem('mtool_language', language);
  }, [language]);

  const t = useCallback((key: TranslationKey, params?: Record<string, string | number>) => {
    let text = translations[language][key] || key;
    if (params) {
      Object.entries(params).forEach(([k, v]) => {
        text = text.split(`{${k}}`).join(String(v));
      });
    }
    return text;
  }, [language]);

  return (
    <I18nContext.Provider value={{ language, setLanguage, t }}>
      {children}
    </I18nContext.Provider>
  );
}

export function useI18n() {
  const context = useContext(I18nContext);
  if (!context) throw new Error('useI18n must be used within I18nProvider');
  return context;
}
