        ...
        <div className="flex-[3] flex flex-col gap-6 overflow-y-auto pr-2">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-2xl">
            ...
            <div className="flex justify-end gap-2">
               <button className="p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded transition-colors border border-transparent hover:border-slate-700" title="Paste from clipboard" onClick={async () => { try { const text = await invoke<string>('read_text_from_clipboard'); setPayload(text); } catch(e){ console.error(e); } }}>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-2xl">
             ...
             <div className="flex items-center gap-4">
                <div className="flex gap-2">
                   <button onClick={() => setSelectedColor('#6366f1')} className={`w-8 h-8 rounded-full bg-[#6366f1] cursor-pointer transition-all ${selectedColor === '#6366f1' ? 'ring-2 ring-offset-2 ring-offset-slate-900 ring-indigo-500' : 'hover:ring-2 ring-offset-2 ring-offset-slate-900 ring-slate-600'}`}></button>
                   <button onClick={() => setSelectedColor('#34d399')} className={`w-8 h-8 rounded-full bg-[#34d399] cursor-pointer transition-all ${selectedColor === '#34d399' ? 'ring-2 ring-offset-2 ring-offset-slate-900 ring-emerald-400' : 'hover:ring-2 ring-offset-2 ring-offset-slate-900 ring-slate-600'}`}></button>
                   <button onClick={() => setSelectedColor('#000000')} className={`w-8 h-8 rounded-full bg-[#000000] border border-slate-700 cursor-pointer transition-all ${selectedColor === '#000000' ? 'ring-2 ring-offset-2 ring-offset-slate-900 ring-slate-400' : 'hover:ring-2 ring-offset-2 ring-offset-slate-900 ring-slate-600'}`}></button>
                </div>
        ...
        <div className="flex-[2] flex flex-col gap-4">
          <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl flex flex-col justify-center items-center relative overflow-hidden shadow-2xl">
             ...
             <div className="relative mt-8 group">
                ...
                <div className="relative w-64 h-64 bg-slate-950 border border-slate-800 rounded-xl p-4 flex items-center justify-center shadow-xl">
                   {qrBase64 ? (
                      <img 
